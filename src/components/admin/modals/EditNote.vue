		...
		<form class="mx-2">
			...
			<div class="form-group my-3">
				...
				<a @click.prevent="() => { $refs.documentInput.value= ''; $refs.documentInput.click() }">
					...
					<span class="text-info">Update document</span>
				</a>
			</div>
			<div class="form-group my-3">
...
<script>
	...
	export default {
		...
		methods:{
			...mapActions(['closeEditModal', 'editNote','clearEditMeta']),
			catchDocument(e){ this.document = e.target.files[0] },
			async submitNote() {
				this.isLoading = true
				try {
					await this.editNote({ note: this.note, document: this.document })
					window.Fire.$emit('NoteEdited',this.note)
					this.closeEditModal()
					new window.Toast({icon: 'success', title: 'Note updated successfully'})
				} catch (error) {
		...
		async mounted(){
			this.note = this.getEditMeta
			this.document = this.note.document
			this.clearEditMeta()
		},
		validations:{
			note: {
