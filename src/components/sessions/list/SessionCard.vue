		...
		<div class="alert my-2" role="alert" :class="{'alert-success': isStillInSession, 'alert-danger': wasCancelled, 'alert-info': !wasCancelled && !isStillInSession}">
			...
			<div v-if="!wasCancelled">
				...
				<div class="small d-flex justify-content-between" v-else>
					...
					<span v-if="getRating">You rated: {{ getRating | toOneDP }}/5.0</span>
				</div>
			</div>
			<div v-else>
...
<script>
	...
	export default {
		...
		computed: {
			...
			getRating(){
				return this.getId === this.session.tutor ? this.session.reviews.tutor.rating : this.session.reviews.student.rating
			},
			...
			getTime(){
				...
				if(hours > 0){
					return `Ends in ${hours} hours ${minutes < 10 ? '0' + minutes : minutes} ${minutes > 1 ? 'minutes' : 'minute'}`
				}else if(minutes > 0){
